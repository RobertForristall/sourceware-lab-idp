...
public class JwtManager {
	 ...
	 * @param clientId        {@link String} ID identifying the application
	 *                        requesting a token
	 * @param audience        {@link String} endpoints that the token can be used
	 *                        for
	 * @param tokenExpiration Time (long) that a token should be valid
	 * @param keyStoreData    {@link IdpKeyStoreData} that contains information
	 *                        related to the keystore
	 ...
	 */
	public static SignedJWT getSignedJwtToken(JWSAlgorithm securityAlg, String clientId, String audience,
			long tokenExpiration, IdpKeyStoreData keyStoreData) throws KeyStoreException, NoSuchAlgorithmException,
			CertificateException, UnrecoverableEntryException, IOException, OperatorCreationException, JOSEException {
		JWK key = IdpKeyStoreAccessor.accessKeyStore(securityAlg, keyStoreData);
		Date now = new Date();
		JWSHeader header = new JWSHeader.Builder((JWSAlgorithm) key.getAlgorithm()).keyID(key.getKeyID())
				.type(JOSEObjectType.JWT).build();
		JWTClaimsSet jwt = new JWTClaimsSet.Builder().issuer(clientId).subject(clientId).audience(audience)
		...
		JWSSigner signer = securityAlg.equals(JWSAlgorithm.RS384) ? new RSASSASigner(key.toRSAKey().toRSAPrivateKey())
	 ...
	 * @throws CertificateException        if there is an issue generating/fetching
	 *                                     the certificate associated with a key
	 * @throws UnrecoverableEntryException if there is an issue getting the specific
	 *                                     key in the keystore
	 * @throws OperatorCreationException   if there is an issue creating a
	 *                                     certificate writer for the key
	 * @throws IOException                 if there is an IO related issue when
	 *                                     trying to access the keystore
	 * @throws JOSEException               If there is an issue creating the token
	 *                                     verifier
	 * @throws ParseException              if there is an issue parsing the provided
	 *                                     JWT
	 */
	public static boolean verifySignedJwtToken(String jwt, JWSAlgorithm securityAlg, IdpKeyStoreData keyStoreData)
			throws KeyStoreException, NoSuchAlgorithmException, CertificateException, UnrecoverableEntryException,
			OperatorCreationException, IOException, JOSEException, ParseException {
		JWK key = IdpKeyStoreAccessor.accessKeyStore(securityAlg, keyStoreData);
		SignedJWT signedJwt = SignedJWT.parse(jwt);
		JWSVerifier verifier = securityAlg.equals(JWSAlgorithm.RS384) ? new RSASSAVerifier(key.toRSAKey())
